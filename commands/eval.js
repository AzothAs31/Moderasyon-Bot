...
exports.run = async (client, message, args) => {
    ...
    let embed = new EmbedBuilder().setColor('#030303').setAuthor({ name: message.guild.name, iconURL: message.guild.iconURL({ dynamic: true }) }).setFooter({ text: `mssix was here`, iconURL: message.author.avatarURL({ dynamic: true }) }).setTimestamp()
    ...
    function clean(text) {
        ...
        text = text.replaceAll(client.token, "yarramin basini alirsin askim bb.");
        return text;
    };
    try {
